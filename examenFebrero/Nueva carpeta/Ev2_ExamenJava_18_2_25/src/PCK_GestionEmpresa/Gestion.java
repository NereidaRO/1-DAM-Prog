...
public class Gestion implements IGestion{
	...
	public boolean crearArticulo() {
		...
		for (int i = 0; i<productos.length; i++) {
			...
			if(tipoProducto.toUpperCase() == "MOTOR") {
				...
				genMarca = sc.nextLine();

				
				Motor prodMotorNew = new Motor(genMarca, idProducto, nombreProducto, cantProducto, precioUnitario, TipoProducto.MOTOR);
				productos[i] = prodMotorNew;
				...
				Ropa prodRopaNew = new Ropa(genMarca, idProducto, nombreProducto, cantProducto, precioUnitario, TipoProducto.ROPA);
				productos[i] = prodRopaNew;
	...
	public boolean crearFactura() {
		...
		sc.nextLine();
		System.out.println("Introduce la fecha de la factura (dd/mm/aa): ");
		fechaFactura = sc.nextLine();
		
		//Factura(int numFactura, String fechaFactura, ArrayList<Producto> listaProductos)
		//meter cada uno de los prods en arrayList y eso pasar a facturas[]
		for (int i = 0; i<productos.length; i++) {
			listaProductos.add(productos[i]);
		}
		
		Factura facturaParaArray = new Factura(idFactura, fechaFactura, listaProductos);
		facturas[0] = facturaParaArray;
		
		sc.close();
		return result;
	};
	public void mostrarFacturas() {
		Factura facturaDesglosar = facturas[0];
		System.out.println("Desglose de la factura: " + facturaDesglosar.getNumFactura());
		System.out.println("Fecha: " + facturaDesglosar.getFechaFactura());
		
		System.out.println(facturas[0].toString());
	}
